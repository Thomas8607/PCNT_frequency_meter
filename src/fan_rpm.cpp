...
static void IRAM_ATTR pcnt_intr_handler(void *arg) {
    unsigned long currentMicros = micros(); //Time at instant ISR was called
...
int countRPM(long firstTime, long lastTime, int pulseTotal, int pulsePerRev) {
  long timeDelta = (lastTime - firstTime);                            // lastTime - firstTime
...
void loop() {
  ...
  long lastStamp0 = 0; //for previous time stamp for fan 0
    ...
    */
    ...
    if (res == pdTRUE) {
      if (evt.unit == 0) { // Fan 0 - TURN THIS BLOCK INTO A FUNCTION THAT TAKES THE FAN OBJECT
        ...
        RPM0 = countRPM(lastStamp0,evt.timeStamp,20,1); // 20, 2
        if (RPM0 == -1){
      ...
      Serial.println("Frekvencia: " + String(RPM0));
    }
  }
  if(user_isr_handle) {
