...
void loop()
{
  ...
  for(;;){
    ...
    */
    ...
    if (res == pdTRUE) {
      //Serial.println("Event PCNT unit: " + String(evt.unit) + "Status: " + String(evt.status));
      if (evt.unit == 0) { // Fan 0 - TURN THIS BLOCK INTO A FUNCTION THAT TAKES THE FAN OBJECT
        if (lastStamp0 == 0){
        lastStamp0 = evt.timeStamp;
        }
        RPM0 = countRPM(lastStamp0,evt.timeStamp,20,1);
        if (RPM0 == -1){
      ...
      Serial.println("Fan 0 RPM: " + String(RPM0) + " Fan 1 RPM: " + String(RPM1) + " Fan 2 RPM: " + String(RPM2));
    }
  }
  if(user_isr_handle) {
